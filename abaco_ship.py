...
# if dists are not ordered, select only one transect
lon_uv = lon_uv[adcp_in]
lat_uv = lat_uv[adcp_in]
time_uv = time_uv[adcp_in]
# for 2011
# max_dist_i = np.where(adcp_dist == np.nanmax(adcp_dist))[0][0] + 1
# adcp_dist = adcp_dist[0:max_dist_i]
# U = U[:, 0:max_dist_i]
# V = V[:, 0:max_dist_i]
# lon_uv = lon_uv[0:max_dist_i]
# lat_uv = lat_uv[0:max_dist_i]
# time_uv = time_uv[0:max_dist_i]
...
ax1.scatter(lon_uv, lat_uv, s=2, color='k')
ax1.grid()

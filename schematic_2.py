...
z_eta = np.where(z_g < -1150)
eta = y_pos + np.zeros(np.size(z_g))
eta[z_eta] = y_pos - 5*np.cos( (2*np.pi/4300)*z_g[z_eta] )
ax.plot3D(-10*np.ones(np.size(z_g)),eta,np.flipud(np.linspace(-5000,0,np.size(z_g))),color='#800000',zdir='z')
...
ax.text(-10,y_pos+12,0,r'$\eta_i$ (z)')
